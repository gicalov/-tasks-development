...
const FallingSnowflake = ({ layerSize, snowflakeOptions, }) => {
    console.log(5);
    const shapeRef = useRef(null);
    const shapePositionRef = useRef(null);
    ...
    const snowflakeSize = (layerSize.width / 1000) * (snowflakeOptions?.snowflakeSize || 10);
    useEffect(() => {
        if (shapeRef.current) {
    ...
    return (_jsx(Shape, { ref: shapeRef, width: snowflakeSize, height: snowflakeSize, offsetX: snowflakeSize / 2, offsetY: snowflakeSize / 2, sceneFunc: (context, shape) => {
            ...
            const drawSnowflake = () => {
                for (let i = 0; i < branchesCount; i++) {
                    ...
                    for (let j = 1; j <= 9; j++) {
                        ...
                        const branchOffsetX = startX + Math.cos(angle + angleOffset) * (branchLength / 3);
                        const branchOffsetY = startY + Math.sin(angle + angleOffset) * (branchLength / 3);
        ...
        }, fill: "#00D2FF", stroke: "#00D2FF", strokeWidth: snowflakeSize / 10 }));
};
